...
| P04 | Canonical roles, hosts, legacy guards | PASSED | VP-P04 PASSED 2026-07-12 (removed-role/feature sweep incl. deps/bundles; legacy URLs 307 to /gone; host routing grants no authz — login still required on correct host; broker_agent not registrable; builder agent uninvitable by schema; flags hard-off for removed capabilities; z.enum validator rejects deleted roles; stale claims quarantined; server HTTP 200). Two defects found+fixed during VP: env-driven maps/whatsapp flags, silent 404 on legacy URL | 2026-07-12 — branch phase/04-roles-hosts-guards; actor model (9 types), invitation-only Broker Agent schema, builder-agents removed, host middleware (broker./builder./account.), SYS states, legacy scanner + runtime guards; 27 unit tests green. **BLOCKED item: supabase db push to kqkrisasxufstdbkniov awaits explicit user approval** |
| P05 | Database, ownership, migration foundation | IMPLEMENTED | VP-P05 pending | 2026-07-12 — branch phase/05-database-foundation; expand-only canonical-entities migration, ADR-005 ownership mapping, guarded synthetic seed, 36 unit tests green. BLOCKED: VP-P05 db-dependent items (1,3,4,6-8,10,11) — supabase db push failed with SASL password-authentication error (provided DB password rejected by server) and retries are permission-blocked; user must verify/reset DB password |
| P06–P17 | See File 07 §6 phase registry | NOT_STARTED | — | Blocked until P05 verification PASSED (DEC-012 still open) |
